...
file_names <- list.files("./yeast_data/")
file_names_without<-tools::file_path_sans_ext(file_names)
for(eqtlset in file_names_without)
{

    yeast_data<-read.table(paste0("./yeast_data/",eqtlset,".csv"),sep=',',header=T,check.names = F)
    #sim_data<-read_data(paste0("./simu_data/",dt,ds,".txt"),inputs=100) #100 is the number of trios 
    n <- nrow(yeast_data)
    # Extract the node/column names
    V <- colnames(yeast_data)
    # Calculate Pearson correlation
    #suffStat_C <- list(C = cor(sim_df), n = n)
    # Calculate Robust correlation 
    robustCor_obj<-RobustCor(yeast_data,Beta=0.005,plot=FALSE)
    ...
    counter=0;
      ...
      # sim_df<-data.frame(L=unlist(sim_data[[i]][1]),A=unlist(sim_data[[i]][2]),B=unlist(sim_data[[i]][3]))
        ...
        yeast_data.mrpc <- MRPC(data = yeast_data,
                            suffStat = suffStat_C,
                            GV = 1,
                            FDR = f,
        ...
        res_list[[counter]]<-names(yeast_data.mrpc@graph@edgeData@data)
        res_values[[counter]]<-unname(unlist(yeast_data.mrpc@graph@edgeData@data))
      }
    ...
    colnames(res_table_trios)<-c("fdr")
    
    
    res_table<-cbind(res_table_trios,res_table_names,res_table_values)
    
    
    write.csv(res_table,file=paste0("./yeast_results/",eqtlset,"mrpc",".csv"),row.names = F,quote = F);
    print(paste0(eqtlset," completed"))
}
