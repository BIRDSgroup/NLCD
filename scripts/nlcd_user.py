...
def nlcd_batch(shared_data, shuffles, algo, reverse=False, sample_seed=None, normal=False, usercall=False,
    ...
    et = time.time()
    elapsed_time = et - st
    mode_str = f"split (train_frac={train_frac})" if split else "no-split"
    print("Algo ",algo," shuffles ",shuffles," datasize = ",len(shared_data[0][0])," reverse ",reverse, " mode ", mode_str)
...
def nlcd_single_for_batch(singletriodata, shuffles, algo, sample_seed=None, verbose=False, reverse=False,
                          normal=False, split=False, train_frac=0.8):
    assert len(singletriodata)==3
    return nlcd_single(
        singletriodata[0], singletriodata[1], singletriodata[2],
        shuffles, algo, sample_seed, verbose, reverse, normal,
...
def nlcd_single(L, A, B, shuffles, algo, sample_seed=None, verbose=True, reverse=False, normal=False,
    ...
    if split:
        # train/test split path: --normal is intentionally not supported
        # (the per-L normalization would leak test-set sigma into preprocessing)
        if normal:
            raise ValueError("--normal is not supported with --split (would leak test-set sigma into preprocessing)")
        if reverse==False:
            out=combine_tests_split(L,A,B,shuffles,algo,train_frac)
        elif reverse==True:
            out=combine_tests_split(L,B,A,shuffles,algo,train_frac)
        else:
