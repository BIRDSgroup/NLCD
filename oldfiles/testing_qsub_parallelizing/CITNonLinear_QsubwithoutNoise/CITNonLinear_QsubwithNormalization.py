...
for i in range(j,j+n): 
    ...
    loss_list_Bresidual=stratify_B_n_times(L,standardize(A_shuffle),standardize(B_shuffle),shuffles) #conditional independence test
    true_LA=compute_loss(L,A)
    true_LB=compute_loss(L,B)
    #print("Next",B_shuffle)
    true_LBresidual=compute_loss(L,residual(standardize(A),standardize(B_shuffle)))
